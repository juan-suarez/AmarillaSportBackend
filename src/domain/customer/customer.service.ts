...
export class CustomerService {
  ...
  async getCustomerByEmail(email: string, returnEntity: boolean = false): Promise<Result<CustomerDto | Customer, string>> {
  ...
  async createCustomer(customer: CreateCustomerDto): Promise<Result<CustomerDto,string>>{ 
    try{
      const newCustomer = this.customerRepository.create(customer)
      
      newCustomer.password = bcrypt.hashSync(customer.password,10)

      return new Success( this.mapToDto(await this.customerRepository.save(customer)));
    } catch(error) {
      console.error(error);
