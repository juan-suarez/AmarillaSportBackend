...
export class TransactionOrchestrator {
  ...
  readonly publicAccesKey: string =
    'pub_stagtest_g2u0HQd3ZMh05hsSgTS2lUV8t3s4mOt7';
  readonly privateAccesKey: string =
  ...
  async createTransaction(
    ...
    const isStockAvialable = products.value.some(
      (product, index) => product.stock >= shoppingCartProdcuts[index].quantity,
    );
    if (!isStockAvialable) {
      return new Failure('Stock unavailable');
    }

    const acceptanceToken = await this.bankService.getAcceptanceToken(
      this.publicAccesKey,
    );
    ...
    const CardToken = await this.bankService.cardTokenization(
      cardDetails,
      this.publicAccesKey,
    );
    ...
    const unsignedSignature =
      transactionNumber + totalAmount + 'COP' + this.integrityKey;
    const signature = crypto
      .createHash('sha256')
      .update(unsignedSignature)
      .digest('hex');
    ...
    const bankTransaction = await this.bankService.CreateTransaction(
      bankCreateTransactionPayload,
      this.privateAccesKey,
    );
    ...
    const transactionDetails =
      await this.transactionDetailService.createTransactionDetails(
    ...
    return new Success({
      transaction_number: transactionValue.transaction_number,
    });
  }
